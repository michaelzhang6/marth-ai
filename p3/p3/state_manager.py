...
class StateManager:
    ...
    def __init__(self, state):
        ...
        self.addresses['80479D30'] = int_handler(
            self.state, 'menu', 0, 0xFF, Menu, Menu.Characters)
        self.addresses['804D6CAC'] = int_handler(
            self.state, 'stage', 8, 0xFF, Stage, Stage.Unselected)
        ...
        for player_id in range(4):
            ...
            self.addresses[add_address(
                '8045310C', 0xE90 * player_id)] = int_handler(player, 'stocks', 8, 0xFF)

            cursor_x_address = add_address('81118DEC', -0xB80 * player_id)
            cursor_y_address = add_address('81118DF0', -0xB80 * player_id)
            self.addresses[cursor_x_address] = float_handler(
                player, 'cursor_x')
            self.addresses[cursor_y_address] = float_handler(
                player, 'cursor_y')

            type_address = add_address('803F0E08', 0x24 * player_id)
            type_handler = int_handler(
                player, 'type', 24, 0xFF, PlayerType, PlayerType.Unselected)
            character_handler = int_handler(
                player, 'character', 8, 0xFF, Character, Character.Unselected)
            self.addresses[type_address] = [type_handler, character_handler]

            data_pointer = add_address('80453130', 0xE90 * player_id)
            self.addresses[data_pointer + ' 70'] = int_handler(
                player, 'action_state', 0, 0xFFFF, ActionState, ActionState.Unselected)
            self.addresses[data_pointer +
                           ' 8C'] = float_handler(player, 'facing')
            self.addresses[data_pointer +
                           ' E0'] = float_handler(player, 'self_air_vel_x')
            self.addresses[data_pointer +
                           ' E4'] = float_handler(player, 'self_air_vel_y')
            self.addresses[data_pointer +
                           ' EC'] = float_handler(player, 'attack_vel_x')
            self.addresses[data_pointer +
                           ' F0'] = float_handler(player, 'attack_vel_y')
            self.addresses[data_pointer +
                           ' 110'] = float_handler(player, 'pos_x')
            self.addresses[data_pointer +
                           ' 114'] = float_handler(player, 'pos_y')
            self.addresses[data_pointer + ' 140'] = int_handler(
                player, 'on_ground', 0, 0xFFFF, lambda x: x == 0, True)
            self.addresses[data_pointer +
                           ' 8F4'] = float_handler(player, 'action_frame')
            self.addresses[data_pointer +
                           ' 1890'] = float_handler(player, 'percent')
            self.addresses[data_pointer +
                           ' 19BC'] = float_handler(player, 'hitlag')
            self.addresses[data_pointer +
                           ' 19C8'] = int_handler(player, 'jumps_used', 0, 0xFF)
            self.addresses[data_pointer + ' 19EC'] = int_handler(
                player, 'body_state', 0, 0xFF, BodyState, BodyState.Normal)
    ...
    def field_copy(self):
        """Returns a list of the important fields of the state manager"""
        op = self.state.players[0]
        ai = self.state.players[2]
        fields = [op.pos_x, op.pos_y, op.action_state.value, op.facing, op.self_air_vel_x, op.self_air_vel_y, op.attack_vel_x, op.attack_vel_y, op.on_ground, op.action_frame, op.percent, op.hitlag, op.jumps_used, op.body_state.value,
                  ai.pos_x, ai.pos_y, ai.action_state.value, ai.facing, ai.self_air_vel_x, ai.self_air_vel_y, ai.attack_vel_x, ai.attack_vel_y, ai.on_ground, ai.action_frame, ai.percent, ai.hitlag, ai.jumps_used, ai.body_state.value]
        return fields
